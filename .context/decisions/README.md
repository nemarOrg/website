...
- ADR 0010 - [Tiered access: base (auto) vs service (admin-gated)](0010-tiered-access-base-service.md) (accepted; auto-approve half superseded by nemar-cli ADR 0040)
- ADR 0011 - [Soften the upload profile gate for existing service-access users](0011-soften-upload-profile-gate.md) (accepted; `block` branch superseded by ADR 0014)
...
- [0017: Site-wide CSP allowances for the Open Science Assistant widget](0017-site-wide-csp-for-the-open-science-assistant.md) - the assistant is embedded on every route, so its script, worker and connect allowances are too; `'unsafe-eval'` deliberately stays scoped to `/dataset/*`
