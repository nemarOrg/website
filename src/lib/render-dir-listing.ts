...
function renderFileRow(
  ...
  if (previewType) {
    ...
    //
    // Signal files take this same branch, and specifically the same
    // `aria-expanded` semantics (website#217). Between #199 and #217 they
    // were `aria-haspopup="dialog"`, because the button opened the viewer
    // straight into a modal; now it expands the adjacent panel like every
    // other preview type, and the modal sits behind a separate "Enlarge"
    // control inside that panel. Only the tooltip still distinguishes them.
    const title =
      previewType === "eeg"
        ? `Open signal viewer for ${esc(entry.name)}`
        : `Preview ${esc(entry.name)}`;
    row.push(
      `<button class="tree__preview-btn" type="button" data-preview-path="${esc(fullPath)}" data-preview-type="${previewType}" data-file-size="${entry.size}" data-file-name="${esc(entry.name)}" aria-expanded="false" title="${title}"><span class="tree__name">${displayName}</span></button>`,
    );
  } else {
  ...
  //
  // Signal rows get a slot too (website#217) — the viewer mounts into it and
  // is only moved into the dialog when the user asks for it. Two differences
  // from a text preview, both marked by `tree__preview--signal`: the panel
  // drops the fixed height cap (the scope's height tracks its width, so a
  // 480px scroller would bury the plot), and it carries no `aria-live`. A
  // live region around a mounted viewer would announce every cursor-readout
  // and legend update; the transient messages that genuinely need announcing
  // carry their own `role="status"` / `role="alert"` instead.
  return [
